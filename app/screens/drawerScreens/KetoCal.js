...
const KetoCal = ({ route }) => {
  ...
  const [targetWeights, setTargetWeight] = useState("");
  const [targetWeightsError, setTargetWeightError] = useState("");
  ...
  const handleTargetChange = (value) => {
    ...
    if (value == "0") {
      setTargetWeightError(
        "Please Enter Greater Than Or Equal " + targetWeights + "."
      );
      setTargetWeight("");
    } else if (isNaN(value) == true) {
      setTargetWeightError("Only Digits Are Allowed.*");
    } else if (value?.includes(".") == true) {
      let check = parseFloat(value.match(/\d+\.\d{1}/));
      if (isNaN(check) == true) {
        setTargetWeight(value);
      } else {
        // setTargetWeight(check);
        if (value >= 0 && value <= 120) {
          setTargetWeight(check);
        } else {
          setTargetWeightError("Invalid TargetWeight. *");
        }
      }
    } else {
      if (
        (parseFloat(value) >= 0 && parseFloat(value) <= 120) ||
        value.length < 3
      ) {
        setTargetWeightError("");
        setTargetWeight(parseFloat(value || 0));
      } else {
        setTargetWeight(120);
        setTargetWeightError("");
      }
      ...
      >
        ...
        >
          ...
          <CalculatorComp
            ...
            inpValue={targetWeights.toString()}
            slidValue={parseInt(targetWeights)}
            error_msg={targetWeightsError}
          />
